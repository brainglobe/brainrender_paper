...
def make():
    ...
    neurons = scene.add(*make_neurons(*neurons, neurite_radius=11))


    # root_box(scene)
    th = scene.add_brain_region("TH", alpha=0.1, color = blue_grey_dark)
    mos = scene.add_brain_region("MOs", alpha=0.07, color = blue_grey)
    ...
    anim = Animation(scene, "videos", "Video2", size=None)

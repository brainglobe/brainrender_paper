...
def make():
    ...
    # root_box(scene)
    th = scene.add_brain_region("TH", alpha=0.3)
    mos = scene.add_brain_region("MOs", alpha=0.3)
    scm = scene.add_brain_region("SCm", alpha=0.3)
    ...
    anim = Animation(scene, "videos", "video_two", size=None)
